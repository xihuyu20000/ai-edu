<template>
  <transition name="bkcd">
    <div class="bkCountDown" v-show="bkCountDownShow">
      <div id="kbCountDownContent" class="kbCountDownContent" ref="kbCountDownContent"></div>
...
export default {
  ...
  methods: {
    countdown() {
      this.timestamp = new Date().getTime() + this.minutes_timeout * 60000
    ...
    init(ele) {
      $.each(['Hours', 'Minutes', 'Seconds'], function(i) {
        $('<div class="count' + this + '">')
          .html(`<div class = "countPos"> 82              <span class="digit static">0</span> 83         </div> 84         <div class="countPos"> 85           <span class="digit static">0</span> 86         </div>`)
          .appendTo($('#' + ele))
        if (this !== 'Seconds') {
    ...
    switchDigit(position, number) {
      ...
      digit
        .before(replacement)
        .removeClass('static')
        .animate({ top: '20px', opacity: 0 }, 'slow', function() {
          digit.remove()
        })
      replacement.delay(100).animate({ top: 0, opacity: 1 }, 'slow', function() {
...
.digit {
  ...
  color: #fff;
  font-size: 24px;
  background: url('http://static.crecgec.com/Kaipiao/countDown.png') 0 0;
}
...
.countDiv {
  ...
  margin-top: 20px;
  margin-left: 10px;
  background: url('http://static.crecgec.com/Kaipiao/countDown1.png') 0 0;
  background-repeat: no-repeat;
  background-size: 100% 100%;
}
.cdEndCon {
