module.exports = (app) => {
  ...
  router.get("/:id", async (req, res) => {
    const auth = await h.findOne(h.vars.T_AUTH, "*", { id: req.params.id });
    h.ok(res, { data: auth });
  });
  ...
  router.put("/:id", async (req, res) => {
    ...
    const auth = await h.update(h.vars.T_AUTH, data, { id: req.params.id });
    h.ok(res, { data: auth });
  });
  ...
  router.post("/", async (req, res) => {
    if (await h.findOne(h.vars.T_AUTH, "*", { label: req.body.label })) {
    ...
    const id = await h.create(h.vars.T_AUTH, data);
    h.ok(res, { data: id });
  });
  ...
  router.delete("/:id", async (req, res) => {
    const id = await h.remove(h.vars.T_AUTH, { id: req.params.id });
    h.ok(res, { data: id });
  });
